...
public class UserController extends BaseServlet{
    ...
    public String check(HttpServletRequest request, HttpServletResponse response) throws SQLException {
        ...
        // 2. 调用业务逻辑判断用户名是否存在
        // UserServiceImpl弄成单例会不会更好
        // 你觉得呢 我不懂啊 就是说UserServiceImpl这个对象是可以重复使用的
        //没必要每次用都new一个出来 你觉得呢 我试试 当然这不是关键
    ...
    public String register(HttpServletRequest request, HttpServletResponse response){
        ...
        // 已经赋值：用户名 密码 邮箱 性别
        // 没有赋值：激活状态 账号类型 激活码
        user.setStatus(Constants.USER_NOT_ACTIVE); // 未激活状态 0 激活 1
        user.setRole(Constants.ROLE_CUSTOMER); // 普通客户 0 管理员 1
        user.setCode(RandomUtils.createActive());

        // 3. 需要处理的属性：密码 md5进行加密处理
